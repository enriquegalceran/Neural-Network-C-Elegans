...
%% ___INI___
        ...
        n_iteraciones = 10000;
        loops_cada_bloque = 1;
        entradas_tiempo = 8; %originalmente 8
        lineas_simultaneas = 1;
        cada_cuantos_guardar = 1000;
        cada_cuantos_limpiar = 20;
        gusano_ini = 2775;
        gusano_fin = 4036;
        gusano_cont = gusano_fin-gusano_ini+1;
        ...
        learning_rate_frecuency = 8000;
        learning_rate_max = 10;
        learning_rate_min = 0.001;
        
        Syn1 = normrnd(0,1/sqrt(inputSize+1),inputSize+1,   hiddenSize1);
        Syn2 = normrnd(0,1/sqrt(hiddenSize1+1),hiddenSize1+1, hiddenSize2);
        Syn3 = normrnd(0,1/sqrt(hiddenSize2+1),hiddenSize2+1, hiddenSize3);
        Syn4 = normrnd(0,1/sqrt(hiddenSize3+1),hiddenSize3+1, hiddenSize4);
        Syn5 = normrnd(0,1/sqrt(hiddenSize4+1),hiddenSize4+1, outputSize);
        gusano = gusano_ini;


for contador = 1:1000000000000000
        ...
        dibujar = 0;
        dibujar_graf = 1;
        err_y = linspace(1, n_iteraciones*loops_cada_bloque*gusano_cont, n_iteraciones*loops_cada_bloque*gusano_cont);
        err_x = linspace(1, n_iteraciones*loops_cada_bloque*gusano_cont, n_iteraciones*loops_cada_bloque*gusano_cont);
        err_m = linspace(1, n_iteraciones*loops_cada_bloque*gusano_cont, n_iteraciones*loops_cada_bloque*gusano_cont);
        ...
        ahora = datetime('now');
        FileName=['backup/manual/',datestr(now, 'dd HH-MM-SS'),'.mat'];
        save(FileName, 'Syn1', 'Syn2', 'Syn3', 'Syn4', 'Syn5', 'tarta', 'ahora', 'elapsedTime', 'entradas_tiempo', 'lineas_simultaneas', 'gusano', 'contador')

        %% TRAINING
        tic
        i = 1;
        for gusano = gusano_ini:gusano_fin
                ...
                if exist(Name_of_file, 'file') == 2
                    ...
                    for n= 1:loops_cada_bloque
                        ...
                        if rem(i,cada_cuantos_guardar) == 0
                            ...
                            ultimo_guardado = i;
                            ahora = datetime('now');
                            filename = sprintf('backup/%s/contador/%i.mat', nombre_archivo, i);
                            ...
                            if rem(i,cada_cuantos_guardar*cada_cuantos_limpiar) == 0
                                for k=1:cada_cuantos_limpiar-1
                                    filename_borrar = sprintf('backup/%s/contador/%i.mat', nombre_archivo, i-cada_cuantos_guardar*k);
