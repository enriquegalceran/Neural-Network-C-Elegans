% Neural Network v10
% Las entradas van a ser continuas para este. Primero me tocará crear las
% funciones adecuadas para la entrada nueva de datos, además de encontrar
% dichos datos...
% NO FUNCIONA
%
...
%     learning_rate = learning_rate_ini;    
    ...
    for m = 1:n_iteraciones
        ...
        if volver_a_empezar == 0
            X = [X(1:end-49),angulos(:,lo)',BiasVector];
            Y = angulos(:,lo+1)';
            lo = lo + lineas_simultaneas;
            if lo+lineas_simultaneas >= len_x
                volver_a_empezar = 1;
            end
        elseif volver_a_empezar == 1
            X = [];
            for p = 1:entradas_tiempo
                X = [X, angulos(:,p)'];            
            end
            Y = angulos(:,entradas_tiempo+1)';
            X = [X, BiasVector];
            lo = 1 + entradas_tiempo;
            volver_a_empezar = 0;
        else
...
%                 Comprobar2;
            ...
            if rem(i,cada_cuantos_guardar) == 0
                ...
                ultimo_guardado = i;
                ahora = datetime('now');
                save(filename, 'Syn1', 'Syn2', 'Syn3', 'Syn4', 'Syn5', 'ahora', 'elapsedTime', 'entradas_tiempo', 'lineas_simultaneas', 'desde', 'hasta','nombre_archivo')
                if dibujar == 2
                    if gcf ~=1
                        figure(1)
                    end
