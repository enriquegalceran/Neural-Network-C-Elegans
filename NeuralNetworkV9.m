...
%             res_er = Y-res;
            ...
            if rem(i,cada_cuantos_guardar) == 0
                ...
                ultimo_guardado = i;
                ahora = datetime('now');
                save(filename, 'Syn1', 'Syn2', 'Syn3', 'Syn4', 'Syn5', 'ahora', 'elapsedTime', 'entradas_tiempo', 'lineas_simultaneas', 'desde', 'hasta', 'tarta','nombre_archivo', 'learning_rate', 'learning_rate_rate', 'learning_rate_frecuency')
                if dibujar == 2
                    if gcf ~=1
                        figure(1)
                    end
